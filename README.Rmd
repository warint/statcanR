...
This is entirely optional: it adds no new package dependencies, and it makes no network request unless you call `statcan_chat()` yourself. It works with any OpenAI-compatible chat-completions endpoint. The endpoint must use `https://` (plain `http://` is accepted only for a loopback host such as `http://localhost`, for a local model), so the key is never sent in cleartext. Configure it once per session:
