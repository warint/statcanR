...
This feature adds no new package dependencies, and it never makes a network request unless you call `statcan_chat()` yourself. It works with any OpenAI-compatible chat-completions endpoint. The endpoint must use `https://` so the key is never sent in cleartext (plain `http://` is accepted only for a loopback host such as `http://localhost`, for a local model). Configure the endpoint and model once per session with `options()`, environment variables, or arguments; supply the API key through the `STATCANR_LLM_API_KEY` environment variable (or the `api_key` argument), since a secret should not be kept in `options()`:
