...
This vignette describes the code architecture of the statcanR  R package. This package allows to access all Statistics Canada' open economic data (formerly known as CANSIM tables) now identified by product IDs (PID) by the new Statistics Canada's Web Data Service.
...
The _tableNumber_ argument simply referes to the table number of the Statistics Canada data table a user wants to collect, such as '14-10-0287-03' for the labour force variables by province, monthly, seasonally adjusted, as an example.
...
To be more precise about the _sqs_statcan_data()_ function, here is below a further code description:   
  ...
  The fifth step consists in unzipping the .Zip file. The unzipping process gives access to two diffrent _.csv_ files, such as the overall data table and the metadata table.
...
This section presents an example of how to use the _statcanR_ R package and its function sqs_statcan_data().    
